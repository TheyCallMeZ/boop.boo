...
// Helper function to remove BOM (Byte Order Mark) from string
    ...
    async loadData() {
        try {
            ...
            // Load static members
            let staticJson = await fs.readFile('static_members.json', 'utf8');
            staticJson = this.removeBOM(staticJson); // Remove BOM if present
            const staticData = JSON.parse(staticJson);
            this.staticMembers = staticData.map(member => new StaticMember(member));
            console.log(`✅ Loaded ${this.staticMembers.length} static members from JSON file`);

            // Create output directories
            await this.createOutputDirectories();
        } catch (error) {
            console.error(`❌ Error loading JSON files: ${error.message}`);
            console.error(`💡 If you're getting JSON parse errors, your file might have a BOM (Byte Order Mark).`);
            console.error(`💡 Try saving your JSON files as "UTF-8 without BOM" in your text editor.`);
            throw error;
        }
    }
    ...
    async scrapeMountsFromPage(member) {
        ...
        try {
            ...
            // Find needed mounts
            const neededMounts = this.mountData.filter(m => 
                !foundMounts.some(f => f.ID === m.ID)
            );

            // Save files
            await this.saveResults(characterName, member, unknownMounts, neededMounts);
    ...
    async saveResults(characterName, member, unknownMounts, neededMounts) {
        ...
        try {
            ...
            console.error(`Error saving files for ${characterName}: ${error.message}`);
        }
    }
    ...
    async copyFilesToDestination() {
        ...
        try {
			...
			const destinationDirectory = '../src/assets/data/static';
			
            // Clear destination directory
            try {
                const files = await fs.readdir(destinationDirectory);
                await Promise.all(files.map(file => 
                    fs.unlink(path.join(destinationDirectory, file))
                ));
    ...
    async run() {
        try {
            ...
            // Process each member
            for (const member of this.staticMembers) {
                try {
...
// Run the scraper
async function main() {
    const scraper = new MountScraper();
    await scraper.run();
}
